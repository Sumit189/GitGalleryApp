...
(async () => {
  try {
    const [tRaw, rRaw, aRaw, adRaw, idsRaw, themeRaw, albumNamesRaw] = await Promise.all([
    ...
    useAppStore.setState({ authToken: token, currentRepo: repo, autoSync, autoDeleteAfterSync, selectedAlbumIds, theme, gallerySource, albumNameCache });
  } catch {}
})();
